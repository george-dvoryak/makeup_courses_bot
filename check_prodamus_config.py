...
try:
    ...
    print(f"\nSuccess / Fail URL (опционально, только для пользовательского редиректа):")
    print(f"  Success: {base_url}/prodamus/success")
    print(f"  Fail:    {base_url}/prodamus/fail")
    print("  * Эти адреса больше не используются ботом и могут быть пустыми.")
    ...
    print(f"\n2. Success / Fail URL (опционально):")
    print(f"   Success: {base_url}/prodamus/success")
    print(f"   Fail:    {base_url}/prodamus/fail")
    print("   Метод: GET")
    print("   Используются только для отображения сообщения пользователю (бот обрабатывает только Result URL)")
